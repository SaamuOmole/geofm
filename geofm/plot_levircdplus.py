...
arr = tifffile.imread(str(img_path))  # returns HxWxC or CxHxW depending on file
# normalize to H,W,C
...
# handle common stacked layout: 6 channels (R1,G1,B1,R2,G2,B2)
if C == 6:
    A_rgb = arr[..., :3]
    B_rgb = arr[..., 3:6]
elif C == 3:
    ...
    # generic: split in half as two timesteps
    t = C // 2
    A_rgb = arr[..., :t]
    B_rgb = arr[..., t:2*t]
